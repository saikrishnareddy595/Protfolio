...
export const projects: Project[] = [
  {
    ...
    artifacts: {
      ...
      architectureDiagram: "/diagrams/llm-data-quality.svg",
      liveDemo: "https://example.com/todo-llm-demo"
    },
    genaiDetails: {
      problem:
        "On-call engineers were receiving high-volume anomaly alerts without enough context to identify root cause quickly.",
      approach: [
        "Prompt structure: system instructions + table metadata + anomaly signals + historical context.",
        "Output contract: strict JSON schema with fields for anomaly type, confidence, suspected upstream causes, and recommended next checks.",
        "Few-shot examples were used to standardize response format across recurring incident patterns."
      ],
      guardrails: [
        "Input sanitization removed sensitive data and masked direct identifiers before prompt submission.",
        "Schema validation enforced machine-readable outputs; invalid outputs triggered retries with constrained prompts.",
        "Fallback path routed low-confidence cases to deterministic rule-based triage and manual review."
      ],
      latencyCostTradeoffs: [
        "Used lightweight models for frequent checks and escalated only hard cases to larger models.",
        "Cached reusable prompt context for repetitive table families to reduce token overhead.",
        "Batching non-urgent anomaly analysis reduced peak-hour cost without affecting critical alerts."
      ],
      outcomes: [
        "Lowered noisy alert volume while preserving true-positive incident detection.",
        "Reduced mean time to triage through structured anomaly summaries.",
        "Improved engineering trust in AI-assisted operations with explicit fallback rules."
      ]
    }
  },
  {
    ...
    artifacts: {
      ...
      architectureDiagram: "/diagrams/aws-lakehouse.svg",
  ...
  },
  {
    ...
    artifacts: {
      ...
      architectureDiagram: "/diagrams/airflow-migration.svg",
  ...
  },
  {
    ...
    artifacts: {
      ...
      architectureDiagram: "/diagrams/kafka-streaming.svg",
